...
// 9.How would you get an array of users who live in 'Mumbai' and have a salary less than 800,000?
 let salaryless=users.filter(user=>user.salary<800000)
 console.log(salaryless);
...
// 13.How would you find the first user who is an admin and lives in 'Jaipur'?
   let firstuserlives=users.find(user=>user.role==='admin'&& user.city==='Jaipur')
   console.log(firstuserlives);
...
let indexkoltalta = users.findIndex(user => user.city === 'Kolkata');
if (index !== -1) {
    users.splice(indexkoltalta, 1);
}
